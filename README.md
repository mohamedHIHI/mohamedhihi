<p align="center">
  <img src="https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExb3E1d2k0N214NjJscWx4YmR2aHZ0bnh2amluaWxteGc2a2NvNHY5YiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/nZ8HKcnfNWCOI/giphy.gif" width="400">
</p>

<p align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.herokuapp.com?font=Orbitron&size=24&pause=1000&color=00FFAA&center=true&vCenter=true&width=600&lines=Hello+World!+I'm+MOHAMED+El+HIHI;Developement+Digital;Site+Web;Front-end+&+back-end" alt="Typing SVG" />
  </a>
</p>

---

## 👋 Profil

Je suis étudiant en première année en Développement Digital au CMC, passionné par la programmation et la création d’applications web et systèmes interactifs.

Je me concentre actuellement sur l’apprentissage des bases solides du développement logiciel (frontend, backend, logique algorithmique) avec une approche pratique basée sur des projets.

---

## 🧠 Objectif

Mon objectif est de devenir développeur full-stack capable de construire des applications web complètes, fiables et bien structurées, en progressant vers des projets plus complexes (API, systèmes temps réel, architecture backend).

---

## 🛠️ Compétences actuelles

**Frontend**

* HTML / CSS
* JavaScript (bases)
* Python / POO

**Backend**

* PHP (bases)

**Bases de données**

* MYSQL / SQLite

**Outils**

* Git / GitHub
* VS Code / pycharm

---

## 🚀 Projets

### 🌐 Portfolio personnel

Site web personnel pour présenter mon profil et mes projets.

🔗 [https://my-portfolio-khaki-pi-90.vercel.app/](https://my-portfolio-khaki-pi-90.vercel.app/)

### 💡 Projets d’apprentissage

Petits projets de pratique pour renforcer mes compétences en logique, JavaScript et développement web.

### 🎮 Initiation au Game Dev

Expérimentation avec la programmation de jeux simples pour comprendre la logique des systèmes interactifs.

---

## 📚 Ce que j’apprends actuellement

* JavaScript avancé
* Structures de données et algorithmes
* Backend avec Node.js
* Bases des API REST
* Amélioration de la logique de programmation

---

## 🎯 Objectif professionnel

Je cherche à progresser vers :

* Stage en développement web
* Projets pratiques en équipe
* Opportunités d’apprentissage en entreprise

---

## 📬 Contact

* Portfolio: [https://my-portfolio-khaki-pi-90.vercel.app/](https://my-portfolio-khaki-pi-90.vercel.app/)
* LinkedIn: [https://linkedin.com/in/mohamed-hihi-4826523b2](https://linkedin.com/in/mohamed-hihi-4826523b2)
* GitHub: [https://github.com/mohamedHIHI](https://github.com/mohamedHIHI)

---

## ⚠️ Note

Ce profil reflète un parcours en apprentissage. Les projets évoluent progressivement avec l’expérience et la pratique.

---

## 💡 Philosophie

Apprendre en construisant. Comprendre avant d’optimiser. Améliorer chaque projet étape par étape.
